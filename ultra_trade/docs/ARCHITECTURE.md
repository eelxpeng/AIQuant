...
- [ ] **Event log durability**: in-memory ring + async persist, vs. synchronous append. Drives the crash-recovery contract. *Forced by: the event-log spec (D0.2).* **Proposed answer in [`adr/1-recorded-log-format.md`](adr/1-recorded-log-format.md): the on-disk format is decided, and D-8 chooses bounded tail loss written off the hot path. Nothing is implemented yet, and what a lost tail means for position is still D4.4.**
- [ ] **Recorded market-data format**: what `adapters/historical` reads. *Forced by: D1.2 and the first non-synthetic backtest.* **Proposed answer in [`adr/1-recorded-log-format.md`](adr/1-recorded-log-format.md): there is no separate format — a recorded session *is* an event log, and a historical feed replays its inbound records.**
...
- [ ] **Position reconciliation policy**: how internal-vs-venue divergence halts trading, and how it resumes. *Forced by: the reconciliation spec (Phase 2, D2.4).* **Partly answered: a divergence halts and is implemented; how an operator *resolves* one is still open, and [`adr/1-crash-recovery-contract.md`](adr/1-crash-recovery-contract.md) (D-4) states that today the answer is that trading stops.**
- [ ] **Crash recovery**: what a restarted session believes about its position and what it may do about it. *Forced by: D4.4.* **Proposed answer in [`adr/1-crash-recovery-contract.md`](adr/1-crash-recovery-contract.md), with a kill-window matrix and a states × operations matrix. Nothing implemented yet; it needs a venue snapshot request, two record kinds, a format version bump, and a `Recovering` state.**
